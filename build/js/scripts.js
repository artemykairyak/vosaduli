...
$(function () {
  ...
  $('.mobile-nav-menu__burger').on('click', function () {
    ...
    $('.mobile-overlay').removeClass('overlay_disabled'); // $('body').addClass('hidden');
  });
  ...
  $('.user-data__text').on('click', function (e) {
    if ($(e.target).parent().is('.user-data__edit') || $(e.target).parent().is('.user-data__edit-svg')) {
      ...
      var close = '<span class="user-data__text_close"><span class="svg user-data__text_close-svg" data-src="/templates/vosaduly/images/icons/uil-check-circle2.svg"></span></span>'; //var save = '<span class="user-data__text_save"><span class="svg user-data__text_save-svg" data-src="images/icons/uil-check-circle.svg"></span></span>'
  ...
  $('select[name="country"]').on('change', function (e) {
    ...
    $('select').trigger('refresh');
    var oldLength = $('select[name="region"]').children('option').length;
  ...
  $('select[name="region"]').on('change', function (e) {
    ...
    $('select').trigger('refresh');
    var oldLength = $('select[name="city"]').children('option').length;
    var timerId = setInterval(function () {
  ...
  $('.blogs-menu__next').on('click', function (e) {
    ...
    if ($('.blogs-menu__trigger_best').hasClass('trigger_active') && $('.blogs-menu__best').find('.blogs-menu__item').length > 4) {
      SidebarScrollDown($('.blogs-menu__best'), '.blogs-menu__item', 600, $('.blogs-menu__best').attr('data-page'), 4, $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled');
    }

    if ($('.blogs-menu__trigger_new').hasClass('trigger_active') && $('.blogs-menu__new').find('.blogs-menu__item').length > 4) {
      SidebarScrollDown($('.blogs-menu__new'), '.blogs-menu__item', 600, $('.blogs-menu__new').attr('data-page'), 4, $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled');
    }
  });
  $('.blogs-menu__prev').on('click', function (e) {
    ...
    if ($('.blogs-menu__trigger_best').hasClass('trigger_active') && $('.blogs-menu__best').find('.blogs-menu__item').length > 4) {
      SidebarScrollUp($('.blogs-menu__best'), '.blogs-menu__item', 600, $('.blogs-menu__best').attr('data-page'), 4, $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled');
    }

    if ($('.blogs-menu__trigger_new').hasClass('trigger_active') && $('.blogs-menu__new').find('.blogs-menu__item').length > 4) {
      SidebarScrollUp($('.blogs-menu__new'), '.blogs-menu__item', 600, $('.blogs-menu__new').attr('data-page'), 4, $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled');
    }
  });
  $('.blogs-menu__trigger').on('click', function (e) {
    ...
    if ($('.blogs-menu__trigger_new').hasClass('trigger_active')) {
      refreshArrows($('.blogs-menu__new'), '.blogs-menu__item', $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled', 4, $('.blogs-menu__new').attr('data-page'));
      $('.blogs-menu__new').removeClass('blogs-menu_hidden');
      $('.blogs-menu__best').addClass('blogs-menu_hidden');
    } else {
      refreshArrows($('.blogs-menu__best'), '.blogs-menu__item', $('.blogs-menu__prev'), $('.blogs-menu__next'), 'blogs-menu__prev_disabled', 'blogs-menu__next_disabled', 4, $('.blogs-menu__best').attr('data-page'));
  ...
  setTimeout(function () {
    if ($('.pub-maininfo__selected-categories').length > 0) {
      $('.pub-maininfo__category-secoundary-inputs-container').find('input[type="hidden"]').each(function (j, item2) {
        $('.pub-maininfo__category-input_main-category option').each(function (i, item1) {
          if ($(item1).val() == $(item2).val()) {
            $('.pub-maininfo__category-input_additional-category li').each(function (k, item3) {
      ...
      $('.pub-maininfo__category-secoundary-inputs-container').find('input[type="hidden"]').each(function (i, item3) {
        if ($(item3).val() != '') {
          $('.pub-maininfo__selected-category').each(function (j, item4) {
  ...
  if ($('input[name="periodmin"]').val() != 0 && $('input[name="periodmax"]').val() != 0) {
    ...
    $('.pub-period__btn-label').text($('[data-order = first]').attr('data-alias') + ' - ' + $('[data-order = second]').attr('data-alias'));
  }
  ...
  $('.pub-maininfo__category-input_main-category .jq-selectbox__dropdown li').on('click', function () {
    ...
    $('.pub-maininfo__category-input_additional-category .jq-selectbox__dropdown li').each(function (i, item) {
      $(item).removeClass('category_selected');
      $(item).removeClass('category_disabled');

      if ($(item).text() == $(_this).text()) {
  ...
  $('.pub-maininfo__category-secoundary-inputs-container').on('click', function (e) {
    ...
    if ($(e.target).is($('.pub-maininfo__category-input_additional-category .jq-selectbox__dropdown li'))) {
      $(e.target).parent().children().each(function (i, item) {
      ...
      if ($(e.target).hasClass('category_selected') && !$(e.target).hasClass('category_disabled')) {
        $('.pub-maininfo__selected-categories').find($('.pub-maininfo__selected-category')).each(function (i, item) {
        ...
        $('.pub-maininfo__category-secoundary-inputs-container').find('input[type="hidden"]').each(function (i, item) {
          if ($(item).val() == '') {
            $('.pub-maininfo__selected-categories').append('<span class="pub-maininfo__selected-category" data-value="' + $optionValue + '">' + $(e.target).text() + '</span>'); // $('.pub-maininfo__selected-categories').attr('data-value', )
      ...
      $(e.target).closest($('.pub-maininfo__category-secoundary-inputs-container')).find($('.jq-selectbox__dropdown li')).each(function (i, item) {
        if ($(item).hasClass('category_selected')) {
          selectedCount++;
        }
      ...
      $(e.target).remove();
      $('.pub-maininfo__category-secoundary-inputs-container input[type="hidden"]').each(function (k, item3) {
        var res = false;

        if ($(item3).val() != '') {
          $('.pub-maininfo__selected-category').each(function (j, item1) {
  ...
  $('.questions__trigger').on('click', function (e) {
    ...
    if ($('.questions__trigger_new').hasClass('trigger_active')) {
      refreshArrows($('.questions__items_new'), '.questions__item', $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled', 5, $('.questions__items_new').attr('data-page'));
      ...
      refreshArrows($('.questions__items_best'), '.questions__item', $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled', 5, $('.questions__items_best').attr('data-page'));
      ...
      refreshArrows($('.questions__items_noanswer'), '.questions__item', $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled', 5, $('.questions__items_noanswer').attr('data-page'));
  ...
  $('.questions__next').on('click', function (e) {
    ...
    if ($('.questions__trigger_new').hasClass('trigger_active') && $('.questions__items_new').find('.questions__item').length > 5) {
      SidebarScrollDown($('.questions__items_new'), '.questions__item', 660, $('.questions__items_new').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }

    if ($('.questions__trigger_best').hasClass('trigger_active') && $('.questions__items_best').find('.questions__item').length > 5) {
      SidebarScrollDown($('.questions__items_best'), '.questions__item', 660, $('.questions__items_best').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }

    if ($('.questions__trigger_noanswer').hasClass('trigger_active') && $('.questions__items_noanswer').find('.questions__item').length > 5) {
      SidebarScrollDown($('.questions__items_noanswer'), '.questions__item', 660, $('.questions__items_noanswer').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }
  });
  $('.questions__prev').on('click', function (e) {
    ...
    if ($('.questions__trigger_new').hasClass('trigger_active') && $('.questions__items_new').find('.questions__item').length > 5) {
      SidebarScrollUp($('.questions__items_new'), '.questions__item', 660, $('.questions__items_new').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }

    if ($('.questions__trigger_best').hasClass('trigger_active') && $('.questions__items_best').find('.questions__item').length > 5) {
      SidebarScrollUp($('.questions__items_best'), '.questions__item', 660, $('.questions__items_best').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }

    if ($('.questions__trigger_noanswer').hasClass('trigger_active') && $('.questions__items_noanswer').find('.questions__item').length > 5) {
      SidebarScrollUp($('.questions__items_noanswer'), '.questions__item', 660, $('.questions__items_noanswer').attr('data-page'), 5, $('.questions__prev'), $('.questions__next'), 'questions__prev_disabled', 'questions__next_disabled');
    }
  });
  $('.authors__trigger').on('click', function (e) {
    ...
    if ($('.authors__trigger_articles').hasClass('trigger_active')) {
      refreshArrows($('.authors__items_articles'), '.authors__item', $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled', 5, $('.authors__items_articles').attr('data-page'));
      $('.authors__items_articles').removeClass('authors__items_hidden');
      $('.authors__items_comments').addClass('authors__items_hidden');
    } else {
      refreshArrows($('.authors__items_comments'), '.authors__item', $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled', 5, $('.authors__items_comments').attr('data-page'));
  ...
  $('.authors__next').on('click', function (e) {
    ...
    if ($('.authors__trigger_articles').hasClass('trigger_active') && $('.authors__items_articles').find('.authors__item').length > 5) {
      SidebarScrollDown($('.authors__items_articles'), '.authors__item', 480, $('.authors__items_articles').attr('data-page'), 5, $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled');
    }

    if ($('.authors__trigger_comments').hasClass('trigger_active') && $('.authors__items_comments').find('.authors__item').length > 5) {
      SidebarScrollDown($('.authors__items_comments'), '.authors__item', 480, $('.authors__items_comments').attr('data-page'), 5, $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled');
    }
  });
  $('.authors__prev').on('click', function (e) {
    ...
    if ($('.authors__trigger_articles').hasClass('trigger_active') && $('.authors__items_articles').find('.authors__item').length > 5) {
      SidebarScrollUp($('.authors__items_articles'), '.authors__item', 480, $('.authors__items_articles').attr('data-page'), 5, $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled');
    }

    if ($('.authors__trigger_comments').hasClass('trigger_active') && $('.authors__items_comments').find('.authors__item').length > 5) {
      SidebarScrollUp($('.authors__items_comments'), '.authors__item', 480, $('.authors__items_comments').attr('data-page'), 5, $('.authors__prev'), $('.authors__next'), 'authors__prev_disabled', 'authors__next_disabled');
    }
  });
  $('.experience__next').on('click', function (e) {
    ...
    if ($('.experience__items').find('.experience__item').length > 6) {
      SidebarScrollDown($('.experience__items'), '.experience__item', 670, $('.experience__items').attr('data-page'), 6, $('.experience__prev'), $('.experience__next'), 'experience__prev_disabled', 'experience__next_disabled');
    }
  });
  $('.experience__prev').on('click', function (e) {
    ...
    if ($('.experience__items').find('.experience__item').length > 6) {
      SidebarScrollUp($('.experience__items'), '.experience__item', 670, $('.experience__items').attr('data-page'), 6, $('.experience__prev'), $('.experience__next'), 'experience__prev_disabled', 'experience__next_disabled');
    }
  });
  ...
  $('.sign-in-modal__show-pass, .sign-up-modal__show-pass').on('click', function () {
    $(this).prev().attr('type') === 'password' ? $(this).prev().attr('type', 'text').addClass('sign-in-modal__show-pass_showed') : $(this).prev().attr('type', 'password').removeClass('sign-in-modal__show-pass_showed');
  });
  ...
  $('.modal__close').on('click', function () {
    if ((!$('.sign-in-modal').hasClass('modal_disabled') || !$('.sign-up-modal').hasClass('modal_disabled') || !$('.forget-password-modal').hasClass('modal_disabled')) && !$('.error-modal').hasClass('modal_disabled')) {
  ...
  $('.period-modal__enter-btn').on('click', function (e) {
    ...
    if (firstMonth && secondMonth) {
      $('[data-order = first]').attr('data-month').length == 1 ? $('input[name="periodmin"]').attr('value', 0 + $('[data-order = first]').attr('data-month')) : $('input[name="periodmin"]').attr('value', $('[data-order = first]').attr('data-month'));
      $('[data-order = second]').attr('data-month').length == 1 ? $('input[name="periodmax"]').attr('value', 0 + $('[data-order = second]').attr('data-month')) : $('input[name="periodmax"]').attr('value', $('[data-order = second]').attr('data-month'));
    }
  ...
  $('.error-modal-btn, .success-modal-btn').on('click', function (e) {
    ...
    if (!$('.sign-in-modal').hasClass('modal_disabled') || !$('.sign-up-modal').hasClass('modal_disabled') || !$('.forget-password-modal').hasClass('modal_disabled')) {
...
function validationModalForm(form) {
  var inputs = $(form).find('.modal__input');
  var ok = true;
...
function cutText(element, len) {
  element.each(function (index, elem) {
    ...
    if (elemText.length > len) {
      var _cutText = elemText.slice(0, len);

      var resultText = _cutText.slice(0, _cutText.lastIndexOf(' ')) + '...';
...
function SidebarScrollDown(container, itemSelector, trackHeight, page, pageSize, arrowPrev, arrowNext, arrowPrevDisabledClass, arrowNextDisabledClass) {
  if (+page <= 2) {
    container.find(itemSelector).css({
      'transform': 'translateY(-' + String(+trackHeight * +page) + 'px)'
    });
  ...
  refreshArrows(container, itemSelector, arrowPrev, arrowNext, arrowPrevDisabledClass, arrowNextDisabledClass, pageSize, container.attr('data-page'));
}
...
function SidebarScrollUp(container, itemSelector, trackHeight, page, pageSize, arrowPrev, arrowNext, arrowPrevDisabledClass, arrowNextDisabledClass) {
  if (page > 1) {
    var translateMatrix = container.find(itemSelector).first().css('transform');
    var translateNum = +translateMatrix.slice(translateMatrix.lastIndexOf(' ') + 1, -1);
    container.find(itemSelector).css({
      'transform': 'translateY(' + String(translateNum + +trackHeight) + 'px)'
    });
...
function refreshArrows(container, itemSelector, arrowPrev, arrowNext, arrowPrewDisabledClass, arrowNextDisabledClass, pageSize, currPage) {
